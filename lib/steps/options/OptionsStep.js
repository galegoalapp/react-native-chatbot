...
class OptionsStep extends Component {
  ...
  onOptionClick({ value, allowMultiselect }) {
    ...
    if (value === "proceed") {
      return this.props.triggerNextStep({
        value,
        optionsSelected: this.state.optionsSelected,
      });
    }
    if (this.state.optionsSelected.indexOf(value) === -1) {
      this.setState({
        optionsSelected: [...this.state.optionsSelected, value],
      });
    } else {
      this.setState({
        optionsSelected: this.state.optionsSelected.filter(
          (item) => item !== value
        ),
      });
    }
  ...
  isHighlighted = (value) => {
    ...
    return this.state.optionsSelected.indexOf(label) !== -1;
  };
  ...
  renderOption(allowMultiselect) {
    return (option) => {
      ...
      const optionElementStyles = [
        bubbleStyle,
        optionStyle,
        allowMultiselect && this.state.optionsSelected.includes(value)
          ? { borderWidth: 2 }
          : {},
      ];
  ...
  render() {
    const {
      step: { options, allowMultiselect = false },
    } = this.props;
