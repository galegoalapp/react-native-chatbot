...
import { H3, Image } from "native-base";
...
class ChatBot extends Component {
  ...
  onButtonPress() {
    ...
    if (!isInvalid) {
      ...
      this.inputRef && this.inputRef.blur && this.inputRef.blur();
      setTimeout(() => {
        this.setState({
          currentStep,
          renderedSteps,
  ...
  triggerNextStep(data) {
    ...
    if (isEnd) {
      ...
      currentStep = Object.assign(
        ...
        {
          user: true,
          message: currentStep.allowMultiselect
            ? data.optionsSelected.join(", ")
      ...
      if (nextStep.user) {
        if (this.props.fetchInitialTextInputValueForStep) {
          this.setState({
            inputValue: this.props.fetchInitialTextInputValueForStep(
              nextStep.id
            ),
          });
        }
  ...
  getInputText = () => {
    ...
    const textWithinLimit = inputValue.substring(0, inputMaxLength);
    const textBeyondLimit = inputValue.substring(inputMaxLength);
    return (
      <>
        <Text>{textWithinLimit}</Text>
        <Text style={{ backgroundColor: "rgba(208, 2, 27, 0.2)" }}>
          {textBeyondLimit}
        </Text>
      </>
    );
  ...
  render() {
    ...
    return (
      <ChatBotContainer
        ...
        style={[
          style,
          { flexGrow: 1, position: "relative", height: height - topOffset },
        ]}
      >
        <ImageBackground
          source={require("./onboarding-bg.png")}
        ...
        >
          ...
          >
            ...
            <H3 style={{ textAlign: "center", flex: 1 }}>{title}</H3>
          ...
          >
            <Footer
              ...
              style={[
                footerStyle,
                {
                  ...
                  padding: 5,
                  paddingBottom:
                    Platform.OS === "ios" ? topOffset + 20 : topOffset + 10,
                  display: "flex",
                  flexDirection: "row",
                  alignContent: "center",
                  alignItems: "center",
            ...
            >
              <View style={{ flexGrow: 1, width: 100 }}>
                <ExpandingTextInput
                  ...
                  style={[
                    textInputStyle,
                    {
                      ...
                      marginLeft: 0,
                      width: "96%",
                      height: Math.max(50, this.state.inputHeight),
                      borderWidth: 1,
                    },
                  ]}
                  className="rsc-input"
                ...
                {editable &&
                inputValue.length > inputMaxLength - inputWarningThreshold ? (
                  <Text
                    style={[
                      { fontSize: 8, left: "90%" },
              ...
              <View>
                <Button
                  ...
                  disabled={!editable || inputValue.length > inputMaxLength}
...
ChatBot.propTypes = {
  ...
  fetchInitialTextInputValueForStep: PropTypes.func,
  ...
  inputWarningThreshold: PropTypes.number,
...
ChatBot.defaultProps = {
  ...
  botDelay: 1000,
  fetchInitialTextInputValueForStep: undefined,
  botFontColor: "#4a4a4a",
  bubbleStyle: {
    backgroundColor: "#fff",
    borderRadius: 25,
